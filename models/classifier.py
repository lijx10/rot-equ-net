...
class Model():
    def __init__(self, opt):
        ...
        if opt.rot_equivariant_no > 1.5:
            self.encoder = rot_networks.RotEncoder(opt)
        else:
        ...
        self.optimizer_encoder = torch.optim.Adam(self.encoder.parameters(),
                                                  ...
                                                  betas=(0.9, 0.999),
                                                  weight_decay=opt.weight_decay_l2)
        ...
        self.pc = torch.FloatTensor(self.opt.batch_size, 3, self.opt.input_pc_num).uniform_()
        self.sn = torch.FloatTensor(self.opt.batch_size, 3, self.opt.input_pc_num).uniform_()
        self.label = torch.LongTensor(self.opt.batch_size).fill_(1)
        self.node = torch.FloatTensor(self.opt.batch_size, 3, self.opt.node_num)
        ...
        if self.opt.gpu_ids[0] >= 0:
            ...
            self.sn = self.sn.to(self.opt.device)
    ...
    def set_input(self, input_pc, input_sn, input_label, input_node, input_node_knn_I):
        ...
        self.sn.resize_(input_sn.size()).copy_(input_sn)
    ...
    def forward(self, is_train=False, epoch=None):
        self.feature = self.encoder(self.pc, self.sn, self.node, self.node_knn_I, is_train, epoch)  # Bx1024
