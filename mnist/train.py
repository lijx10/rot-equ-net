...
from models.classifier_mnist import Model
...
if __name__=='__main__':
    ...
    testset = MNIST_Loader(opt.dataroot, 'validation', opt)
    testloader = torch.utils.data.DataLoader(testset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.nThreads)
    print('#validation point clouds = %d' % len(testset))
    ...
    for epoch in range(301):
        ...
        # small learning rate at the beginning
        warmup_epoch = 2
        ...
        if epoch >= 0 and epoch%1==0:
            ...
            # save models
            if ((model.test_accuracy.item() > best_accuracy - 0.0011) and (model.test_accuracy.item() > 0.9)) or (
                        epoch % 10 == 0 and epoch > 0):
                print("Saving network...")
                model.save_network(model.encoder, 'gpu_%d_encoder' % opt.gpu_ids[0], '%d_%f' % (epoch, model.test_accuracy.item()), opt.gpu_ids[0])
                model.save_network(model.classifier, 'gpu_%d_classifier' % opt.gpu_ids[0], '%d_%f' % (epoch, model.test_accuracy.item()), opt.gpu_ids[0])
