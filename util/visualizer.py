    ...
    def display_current_results(self, visuals, epoch, iter=0):
        if self.display_id > 0: # show images in the browser
            ...
            for label, item in visuals.items():
                if 'pc' in label:
                    pc_np = np.transpose(item)
                    pc_color_np = np.repeat(np.expand_dims(np.array([91, 91, 91], dtype=np.int64), axis=0),
                                            pc_np.shape[0],
                                            axis=0)  # 1x3 -> Nx3
                    self.vis.scatter(pc_np,
                                     Y=None,
                                     opts=dict(title=label, markersize=2, markercolor=pc_color_np),
